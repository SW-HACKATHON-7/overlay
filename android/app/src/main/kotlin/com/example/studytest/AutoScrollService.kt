...
class AutoScrollService : AccessibilityService() {
    ...
    private fun isSimilarImage(path1: String, path2: String, threshold: Double = 0.995): Boolean {
        return try {
            val file1 = File(path1)
            val file2 = File(path2)

            // 파일 크기로 먼저 빠른 체크 (10% 이상 차이나면 다른 이미지)
            val size1 = file1.length()
            val size2 = file2.length()
            val sizeRatio = minOf(size1, size2).toDouble() / maxOf(size1, size2).toDouble()

            if (sizeRatio < 0.9) {
                Log.d(TAG, "Size diff too large: $sizeRatio")
                return false
            }
            ...
            val similarity = matchingBytes.toDouble() / bytes1.size.toDouble()
            Log.d(TAG, "Image similarity: ${String.format("%.4f", similarity)}")
